...
<div id="container">
  ...
  <div id="sidebar1">
    <div id="subsidebar1">
      ...
      <div id="subsidebar2"><a href="admlogout.php" onclick="isConfirmlog();" >Log out</a>
      </div>
    </div>
    <!-- end #sidebar1 -->
  </div>
...
<!-- Code for inserting into data base -->
 <?php
  ...
  {
   $get= mysqli_query($con,"SELECT * FROM t_admin_mst WHERE adm_username= '$username' ")or die(mysqli_error($con));
   $num= mysqli_num_rows($get);
   while ($row=mysqli_fetch_assoc($get))
//   for($i=0;$i<$num;$i++)
   {
//    $old_password= @mysql_result($get,$i,'adm_password');
    $old_password=$row["adm_password"];
   }
   $old=$_POST['txt_old'];
   ...
   if($old_password == $old){
    if($old == $password) {
      ...
      $query = mysqli_query($con,"UPDATE t_admin_mst SET adm_password='$password' WHERE adm_username='$username' ")
                           or die(mysqli_error());

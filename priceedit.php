...
<div id="container">
 ...
 <?php
  $get= mysqli_query($con,"SELECT * FROM t_price_mst WHERE username= '$username' AND prd_id='$pid' ")or die(mysqli_error($con));
...
//   $pdiscount= @mysql_result($get,$i,'price_discount_type');
 ...
 ?>
 <div id="middletxt">
        <form action="priceedit.php?prid=<?=$prid?>" method="post" name="frm_price" id="frm_price" enctype="multipart/form-data">
...
if($securimage->check($_POST['captcha_code']) == false){
    ...
    if (isset($_POST['submitMain'])) {
        ...
        $pdis = (double)$_POST['txtprc_disprc'];
        $pdiscount = $_POST['rdo_ses'];

        $get = mysqli_query($con, "select * from t_price_mst where username='$username' AND prd_id = '$pid' ");
        $query = mysqli_query($con, "UPDATE t_price_mst SET price_actual='$pact',price_discount='$pdis',
                       price_discount_type='$pdiscount' WHERE username='$username' AND prd_id='$pid'  ") or die(mysqli_error($con));

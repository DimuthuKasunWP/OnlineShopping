...
<div id="wrapper">
 <?php
   $get_catgry=mysqli_query($con,"SELECT DISTINCT(prd_cat) FROM t_product_mst WHERE username = '$username' AND prd_status='Available' ")or die(mysqli_error());
...
<?php
	...
	$query=mysqli_query($con,"SELECT COUNT(*) as num FROM t_product_mst WHERE username= '$username' AND prd_cat= '$cat' AND prd_status='Available' ");
...
?>
<?php
    $get= mysqli_query($con,"SELECT * FROM t_product_mst WHERE username= '$username' AND prd_cat= '$cat' AND prd_status='Available' LIMIT $start, $limit ")or die(mysqli_error());
...
//     $psep= @mysql_result($get,$i,'prd_sep');
 ...
 <?php
    $get_price= mysqli_query($con,"SELECT * FROM t_price_mst WHERE username= '$username' AND prd_id='$prid' ")or die(mysqli_error());
...
//                $pqtyavb= @mysql_result($get,$i,'prd_qtyavb'); // avaiable qty
                $pqtyavb=$rowsp["prd_qtyavb"];
                 $get_price= mysqli_query($con,"SELECT * FROM t_price_mst WHERE username= '$username' AND prd_id='$pid' ")or die(mysqli_error());
                 $num_price = mysqli_num_rows($get_price);
                 while ($rows=mysqli_fetch_assoc($get_price))
//                 for($j=0;$j<$num_price;$j++)
                 {
...
//		  $pdiscount= @mysql_result($get_price,$j,'price_discount_type');
	      ...
	      $query = mysqli_query($con,"INSERT INTO t_cart_temp (s_id,username,prd_id,cart_name,cart_img,cart_qty,cart_qtyavb,cart_qtyordered,cart_act,cart_dis,cart_price)
                       VALUES ('$sid','$username','$pid','$plname','$pimg','$pqty','$pqtyavb','$qtyval','$pact','$pdis','$p_price')") or die(mysqli_error());
              $num=mysqli_num_rows($query);
	    }  
	  }
